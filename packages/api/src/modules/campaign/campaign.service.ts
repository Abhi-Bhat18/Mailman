...
export class CampaignService implements OnModuleInit {
  ...
  async getCampaignAnalytics(campaign_id: string) {
    const views = await this.db
      ...
      .select([
        sql`EXTRACT(DOW FROM opened_at)`.as('day_of_week'), // Extracts the day of the week
        sql`COUNT(*)`.as('view_count'), // Counts the number of views
      ])
      .groupBy('day_of_week') // Group by the day of the week
      .orderBy('day_of_week') // Optionally order by day of the week
      .execute();
    const clicks = await this.db
      .selectFrom('email_clicks')
      .where('campaign_id', '=', campaign_id)
      .select([
        sql`EXTRACT(DOW FROM clicked_at)`.as('day_of_week'), // Extracts the day of the week
        sql`COUNT(*)`.as('view_count'), // Counts the number of views
      ])
      .groupBy('day_of_week') // Group by the day of the week
      .orderBy('day_of_week') // Optionally order by day of the week
