...
export function ViewsLineChart() {
  ...
  const { data, isLoading, isError } = useGetLineChartDataQuery();
  console.log("Data", data);

  const total = React.useMemo(
    () => ({
      views: data ?  data.reduce((acc, curr) => acc + curr.views, 0) : 0,
      clicks:  data ? data?.reduce((acc, curr) => acc + curr.clicks, 0) : 0,
    }),
    [data]
  );
